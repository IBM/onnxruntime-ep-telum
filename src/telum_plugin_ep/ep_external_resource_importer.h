...
/**
 * @brief Telum scaffold derived handle for imported external memory.
 *
 ...
 */
struct TelumExternalMemoryHandle : OrtExternalMemoryHandle {
  std::unique_ptr<char[]> simulated_ptr;  ///< Simulated mapped pointer (CPU memory for testing)

  TelumExternalMemoryHandle(const OrtExternalMemoryDescriptor& descriptor_in)
  ...
  ~TelumExternalMemoryHandle() = default;

  static void ORT_API_CALL ReleaseCallback(_In_ OrtExternalMemoryHandle* handle) noexcept {
    if (handle == nullptr) return;
    delete static_cast<TelumExternalMemoryHandle*>(handle);
  }
};

/**
 * @brief Telum scaffold derived handle for imported external semaphore.
 *
 ...
 */
struct TelumExternalSemaphoreHandle : OrtExternalSemaphoreHandle {
  std::atomic<uint64_t> value;  ///< Simulated fence value for testing

  TelumExternalSemaphoreHandle(const OrtExternalSemaphoreDescriptor& descriptor_in)
      : value(0) {
    // Initialize base struct fields
    version = ORT_API_VERSION;
    ep_device = nullptr;
  ...
  static void ORT_API_CALL ReleaseCallback(_In_ OrtExternalSemaphoreHandle* handle) noexcept {
    if (handle == nullptr) return;
    delete static_cast<TelumExternalSemaphoreHandle*>(handle);
  }
};

/**
 * @brief Telum scaffold implementation of OrtExternalResourceImporterImpl.
 *
 ...
 */
class TelumExternalResourceImporter : public OrtExternalResourceImporterImpl {
 public:
  TelumExternalResourceImporter(const ApiPtrs& apis);
