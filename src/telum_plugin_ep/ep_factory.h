...
/// <summary>
/// Telum scaffold EP factory that can create an OrtEp and return supported device details.
/// </summary>
class TelumEpFactory : public OrtEpFactory, public ApiPtrs {
 public:
  TelumEpFactory(const char* ep_name, ApiPtrs apis, const OrtLogger& default_logger);
  ...
  TelumBackend& CustomOpBackend() const {
    return *custom_op_backend_;
  }
 ...
 private:
  ...
  const std::string vendor_{"IBM"};         // EP vendor name
  const uint32_t vendor_id_{0x1014};        // IBM PCI vendor ID
  ...
  std::unique_ptr<TelumDataTransfer> data_transfer_impl_;  // data transfer implementation for this factory
  std::unique_ptr<TelumBackend> custom_op_backend_;          // backend used by custom op kernels

  std::vector<Ort::CustomOpDomain> custom_op_domains_{2};
  std::vector<std::vector<std::unique_ptr<TelumEpCustomOp>>> created_custom_op_lists_{2};
};
